...
Uses LCEL (LangChain Expression Language) rather than the legacy `LLMChain`,
...
Key Points:"""
    ...
    def summarize_document(
        ...
        docs = retriever.invoke("summary of the entire document")
        docs = [doc for doc in docs if doc.metadata.get("doc_id") == doc_id]
        ...
        # Generate summary (with Groq multi-key failover on rate limits)
        max_len = max_length or SUMMARY_MAX_LENGTH
        summary_prompt = self._create_summary_prompt(max_len)
        text_input = full_text[:10000]  # Limit text length
        ...
        summary_result = LLMService.run_with_failover(_run_summary)
        ...
        def _run_key_points():
            llm = LLMService.get_llm()
            chain = key_points_prompt | llm
            response = chain.invoke({"text": text_input})
            return response.content if hasattr(response, "content") else str(response)

        key_points_result = LLMService.run_with_failover(_run_key_points)
